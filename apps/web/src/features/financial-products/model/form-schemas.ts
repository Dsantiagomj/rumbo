...
// ---------------------------------------------------------------------------
// Form schema for creating a financial product
// ---------------------------------------------------------------------------
// We intentionally do NOT reuse `createProductSchema` from @rumbo/shared here
// because its `metadata` field uses a union-based `productMetadataSchema` that
// cannot discriminate by product type at parse time. Instead, we accept a loose
// record for metadata and use `.superRefine()` to validate it against the
// correct schema from `PRODUCT_TYPE_METADATA_MAP[type]`.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Product type labels (user-facing, Spanish)
// ---------------------------------------------------------------------------
...
export const PRODUCT_TYPE_LABELS: Record<ProductType, { label: string; description: string }> = {
  ...
  checking: {
    ...
    description: 'Cuenta bancaria corriente',
  },
  credit_card: {
    label: 'Tarjeta de Credito',
    description: 'Tarjeta de credito bancaria',
  },
  ...
  loan_mortgage: {
    ...
    description: 'Prestamo para vivienda',
  },
  investment_cdt: {
    label: 'CDT',
  ...
  investment_fund: {
    ...
    description: 'Fondo de inversion colectiva',
  },
  investment_stock: {
    label: 'Acciones',
...
// ---------------------------------------------------------------------------
// Metadata field configuration (for rendering dynamic form fields)
// ---------------------------------------------------------------------------
...
export const METADATA_FIELD_CONFIG: Record<string, MetadataFieldConfig> = {
  ...
  units: {
    label: 'Unidades/acciones',
    placeholder: '100',
    type: 'number',
  },
  broker: {
    label: 'Corredor/Intermediario',
    placeholder: 'tyba',
    type: 'text',
  },
};
